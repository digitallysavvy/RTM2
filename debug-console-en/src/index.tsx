...
import logo from 'assets/agora-en.png';
...
function App() {
  ...
  const setupRtm = (userId: string, config = DEFAULT_RTM_CONFIG) => {
    ...
    try {
      if (!config.appId) {
        message.error('Please enter APP ID');
        throw new Error('Please enter appId');
      }
  ...
  useEffect(() => {
    // TODO: You can modify the user list as needed, change to fixed users, default to only one user, and randomly generate a 4-character string
